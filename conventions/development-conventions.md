# Development Conventions (Default)

**Status:** Default baseline conventions for projects started with this recipe set
**Provenance:** Derived from the ftn project constitution (`docs/constitution.md`), generalized for reuse
**Override:** Projects MAY bind a different conventions file via the `conventions_file` parameter of `design-project`. When they do, that file is authoritative for the project; this file remains the default.
...
- New technology classes not covered here (e.g. native mobile, embedded, ML training) are **permitted** — the constitution does not forbid them. It only governs the areas it explicitly covers, and asks that new patterns be documented (see §7 Governance).
...
- Quality gates (§2) are enforced in the pipeline and block merge/deploy on failure.
...
- **Immutability:** The conventions bound to a project are authoritative. Individual projects MUST NOT silently override them.
...
- **Amendments:** Changes to the default conventions follow semantic versioning:
